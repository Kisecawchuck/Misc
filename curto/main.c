...
#include "jogadores/jogador_17076671.h"
...
void informar_maos_para_jogadores(int rodada, const Rodada* r) {
    ...
    nova_rodada_17076671(rodada, r->carta_virada, r->cartas_por_jogador, r->maos[3]);
}
...
void coletar_apostas(Rodada* r) {
    ...
    for (int i = 0; i < NUM_JOGADORES; i++) {
        ...
        else if (j == 3) r->apostas[j] = apostar_17076671(r->apostas);
...
int processar_jogadas(Rodada* r, Jogada* jogadas) {
    ...
    for (int i = 0; i < jogo.num_jogadores; i++) {
        ...
        else if (j == 3) idx = jogar_17076671(NULL, 0, r->vitorias[j]);
